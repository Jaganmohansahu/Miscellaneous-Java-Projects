...
public class find extends JFrame implements ActionListener
{
	...
	public find(textEditorGui mynote)
	{
		super("Find/Replace");
		samp = mynote;
	...
	public void find()
	{
		int select_start = samp.textArea.getText().indexOf(tf.getText());
		...
		if(select_start == samp.textArea.getText().lastIndexOf(tf.getText()))
		{
			startIndex = 0;
		}

		int select_end = select_start+tf.getText().length();
		samp.textArea.select(select_start, select_end);
	}

	public void find_next()
	{
		String selection = samp.textArea.getSelectedText();
		...
		try
		{
			int select_start = samp.textArea.getText().indexOf(selection, startIndex);
			int select_end = select_start+selection.length();
			samp.textArea.select(select_start, select_end);
			startIndex = select_end+1;

			if(select_start == samp.textArea.getText().lastIndexOf(selection))
			{
				startIndex = 0;
			}
	...
	public void replace()
	{
		...
		{
			find();
			samp.textArea.replaceSelection(tr.getText());
		}
	...
	{
		samp.textArea.setText(samp.textArea.getText().replaceAll(tf.getText(), tr.getText()));
	}
}